...
try{
  ...
  const state={
    ...
    attackCd:0,skillCd:0,skill2Cd:0,rollCd:0,target:null,lockedTarget:null,autoCombat:false,autoTimer:0,autoSkillTimer:0,
  ...
  function update(dt){
    ...
    if(state.autoCombat&&!state.uiOpen){
      ...
      const autoTarget=state.lockedTarget&&!state.lockedTarget.dead
        ?state.lockedTarget
        :OD.findNearestEnemy(world,12);
      ...
      if(autoTarget){
        ...
        if(dist>2.15){
          ...
          if(state.autoSkillTimer<=0&&state.skillCd<=0&&state.mp>=25){
            state.skillCd=5;
            state.mp-=25;
            world.player.attackTime=.42;
            shoot(60,11);
