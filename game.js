...
try{
  ...
  function enemyPersonalSpace(enemy){
    if(enemy?.boss)return 3.6;
    if(["iceGolem","sandTitan","kingBeetle"].includes(enemy?.kind))return 3.0;
    if(["beetle","slug","scorpion"].includes(enemy?.kind))return 2.75;
    return 2.45;
  }
...
persist();
  ...
  function update(dt){
    ...
    document.body.dataset.region=state.currentRegion;
    ...
    if(state.autoCombat){
      ...
      if(autoTarget){
        ...
        const desiredRange=enemyPersonalSpace(autoTarget);
    ...
    // ===== PLAYER / ENEMY SEPARATION V3.1.2 =====
    // Keep bodies from fully clipping, but do not push so hard that melee attacks never connect.
    for(const e of world.enemies){
      if(e.dead)continue;
      const dx=world.player.pos.x-e.pos.x;
      const dz=world.player.pos.z-e.pos.z;
      const d=Math.hypot(dx,dz)||.001;
      const minD=(e.boss?0.98:0.72);
      if(d<minD){
        const push=(minD-d)*.22;
        world.player.pos.x+=dx/d*push;
        world.player.pos.z+=dz/d*push;
      }
    }


    // ===== ENEMY CROWD SEPARATION V3.1 =====
    for(let i=0;i<world.enemies.length;i++){
      ...
      for(let j=i+1;j<world.enemies.length;j++){
        ...
        const d=Math.hypot(dx,dz)||.001;
        const minD=(a.boss||b.boss)?1.6:1.0;
        if(d<minD){
          const push=(minD-d)*.18;
          a.pos.x+=dx/d*push;
          a.pos.z+=dz/d*push;
          b.pos.x-=dx/d*push;
          b.pos.z-=dz/d*push;
    ...
    for(const e of world.enemies){
      ...
      const meleeRange=e.boss?1.55:1.28;
      const attackInterval=e.boss?.95:1.05;
      ...
      if(d<=aggro){
        if(d>meleeRange){
          ...
          // Face-press melee range: stop, stay close, and actually deal damage.
          e.cooldown-=dt;
          if(e.cooldown<=0){
            e.cooldown=attackInterval;
  ...
  function render(){
    ...
    const desiredCamY=(state.currentRegion==="frostHighlands"?9.4:8.4)+(cameraDistance-11.5)*.18;
    ...
    // Frostleaf Highlands
    renderer.drawBox(0,-.16,-31,18,.22,16,[.82,.92,.95,1]);
    // Snow cap layer: intentionally bright so Frostleaf is unmistakable.
    renderer.drawBox(0,.02,-31,17.6,.035,15.6,[.93,.98,1,1]);
    ...
    // Ice road through Frostleaf: overlays the brown road from z=-16 to z=-46.
    for(let z=-16;z>=-46;z-=2.4){
      renderer.drawBox(0,.045,z,1.35,.045,1.2,[.66,.84,.90,1]);
      renderer.drawBox(-1.55,.08,z,.10,.08,1.0,[.90,.98,1,1]);
      renderer.drawBox(1.55,.08,z,.10,.08,1.0,[.90,.98,1,1]);
    }
    ...
    // ===== FROSTLEAF VISIBLE FIX V3.1.1 =====
    // Large frozen lake visible from normal gameplay camera.
    renderer.drawBox(7,.06,-28,5.4,.055,3.6,[.32,.70,.86,1],-.12);
    renderer.drawBox(7,.10,-28,4.8,.025,3.0,[.66,.90,.97,.72],-.12);

    // Giant crystal forest around the main route.
    const frostCrystals=[
      [-8,-24,1.8],[-11,-29,2.4],[-9,-35,2.0],[-5,-40,2.8],
      [8,-23,2.1],[11,-31,2.7],[9,-37,2.3],[5,-43,3.0]
    ];
    for(const [x,z,h] of frostCrystals){
      renderer.drawBox(x,h*.52,z,.34,h*.52,.34,[.52,.85,.96,1],.42);
      renderer.drawBox(x+.35,h*.35,z+.25,.18,h*.35,.18,[.78,.95,1,1],-.35);
    }

    // Mountain gate leading to the boss arena.
    renderer.drawBox(-4.8,2.0,-38,1.2,2.2,2.6,[.62,.78,.84,1],.08);
    renderer.drawBox(4.8,2.0,-38,1.2,2.2,2.6,[.62,.78,.84,1],-.08);
    renderer.drawBox(0,4.1,-38,5.2,.55,1.25,[.72,.88,.93,1]);

    // Circular boss arena floor, very visible.
    for(let i=0;i<20;i++){
      const a=i/20*Math.PI*2;
      const r=6.3;
      const x=Math.cos(a)*r;
      const z=-42+Math.sin(a)*r;
      renderer.drawBox(x,.10,z,.72,.07,1.15,[.72,.90,.95,1],a);
    }
    renderer.drawBox(0,.075,-42,5.1,.055,5.1,[.76,.90,.94,1]);

    // Massive ice cave mouth on the west wall.
    renderer.drawBox(-13,2.2,-33,3.0,2.4,1.4,[.58,.76,.83,1],.05);
    renderer.drawBox(-13,2.1,-33,1.65,1.7,1.55,[.08,.18,.26,1],.05);

    // ===== FROSTLEAF POLISH V3.1 =====
    for(let i=0;i<14;i++){
      const x=-15+(i%7)*5;
      const z=-38+Math.floor(i/7)*12;
      renderer.drawBox(x,-.02,z,2.1,.03,2.1,[.86,.95,.97,1],i*.08);
    }

    renderer.drawBox(-11,1.2,-39,2.7,1.4,.8,[.50,.72,.80,1]);
    renderer.drawBox(-11,1.5,-39,1.4,1.0,.95,[.18,.30,.38,1]);

    for(let i=0;i<12;i++){
      const a=i/12*Math.PI*2;
      const x=Math.cos(a)*6;
      const z=-42+Math.sin(a)*6;
      renderer.drawBox(x,.35,z,.25,.5,.25,[.62,.87,.95,1],a);
    }

    renderer.drawBox(-16,1.6,-31,1.2,2.0,10,[.62,.78,.84,1]);
    renderer.drawBox(16,1.6,-31,1.2,2.0,10,[.62,.78,.84,1]);
    ...
    if(Math.abs(world.player.pos.x)<20 && Math.abs(world.player.pos.z)<20){
          // Main dirt path
          for(let i=-9;i<=9;i++){
