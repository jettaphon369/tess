...
try{
  ...
  const state={
    ...
    attackCd:0,skillCd:0,skill2Cd:0,rollCd:0,target:null,lockedTarget:null,autoCombat:false,autoSkill:true,autoTimer:0,autoSkillTimer:0,autoAnchor:null,autoRange:10,
  ...
  function setUiOpen(open){
    state.uiOpen=open;
    if(open&&state.autoCombat)setAutoCombat(false);
    if(open){
  ...
  document.getElementById("bagBtnSide").onclick=openInventory;
  document.getElementById("questToggleBtn").onclick=()=>{
  ...
  document.getElementById("shopQuickBtn").onclick=()=>{
  ...
  document.getElementById("questQuickBtn").onclick=()=>{
  ...
  function setAutoCombat(enabled){
    ...
    showAutoToast(enabled?"โหมดออโต้เริ่มทำงาน":"ปิดโหมดออโต้");
  }
  ...
  function update(dt){
    ...
    if(state.autoCombat&&!state.uiOpen){
      state.autoTimer-=dt;
      state.autoSkillTimer-=dt;

      const currentLocked=state.lockedTarget&&!state.lockedTarget.dead?state.lockedTarget:null;
      const autoTarget=currentLocked||chooseSmartAutoTarget();
    ...
    for(let i=world.drops.length-1;i>=0;i--){
      ...
      if(Math.hypot(dx,dz)<1.25){
        if(drop.kind==="potion"){
          ...
          flash("เก็บเหรียญ +" + (drop.value*3));addSystemLog("ได้รับเหรียญ "+(drop.value*3));
        }
    ...
    if(state.hp<=0){
      if(state.autoCombat)setAutoCombat(false);
      state.hp=state.maxHp;
    ...
    const inCombat=!!(activeCombatTarget&&!activeCombatTarget.dead);
    document.body.classList.toggle("in-combat",inCombat);
    document.querySelector(".quest-card")?.classList.toggle("combat-hide",inCombat);
    document.querySelector(".minimap")?.classList.toggle("combat-hide",inCombat);
    document.querySelector(".utility-rail")?.classList.toggle("combat-hide",inCombat);
    ...
    if(activeCombatTarget&&!activeCombatTarget.dead){
      ...
      const nm=activeCombatTarget.boss?"บอสตั๊กแตนยักษ์":(activeCombatTarget.kind==="beetle"?"ด้วงเกราะ":"ตั๊กแตน");
  ...
  function render(){
    ...
    for(const e of world.enemies){
      ...
      const base1=flash?[1,.82,.82,1]:(e.kind==="beetle"?[.24,.12,.08,1]:[.34,.62,.20,1]);
      const base2=flash?[1,.62,.62,1]:(e.kind==="beetle"?[.36,.20,.12,1]:[.46,.72,.27,1]);
